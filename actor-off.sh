...
echo "$path/orca-server-mahimahi-off $port $path ${period} ${first_time} $scheme $id $down $up $latency $log $finish_time $qsize $max_it"

$path/orca-server-mahimahi-off $port $path ${period} ${first_time} $scheme $id $down $up $latency $log $finish_time $qsize $max_it
